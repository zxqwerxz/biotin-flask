...
from flask import current_app, render_template, request, flash, send_file
...
@misc.route('/genotyping', methods=['GET', 'POST'])
def genotyping():
    ...
    if len(os.listdir(os.path.join(current_app.config['UPLOAD_FOLDER'], 'ref_snp_genotyping'))) < 1:
        ref_exists = False
    else:
        ref_exists = True
    filenames = os.listdir(os.path.join(current_app.config['UPLOAD_FOLDER'], 'ref_snp_genotyping'))

    # Render an empty form for GET request
    if request.method == 'GET':
        return render_template('misc/genotyping.html', ref_exists=ref_exists, filenames=filenames)
    ...
    if ref_sel == 'None':
        ...
        # Save the file
        file_path = os.path.join(current_app.config['UPLOAD_FOLDER'], 'ref_snp_genotyping', secure_filename(ref[0].filename))
        if secure_filename(ref[0].filename) in os.listdir(os.path.join(current_app.config['UPLOAD_FOLDER'], 'ref_snp_genotyping')):
            os.remove(file_path)
        ...
        list_snp = load_workbook(os.path.join(current_app.config['UPLOAD_FOLDER'], 'ref_snp_genotyping', ref_sel))
    ...
    # Save the zip file
    try:
        if 'genotyping_results.zip' in os.listdir(current_app.config['UPLOAD_FOLDER']):
            os.remove(os.path.join(current_app.config['UPLOAD_FOLDER'],'genotyping_results.zip'))
    except:
        delete_files(f)
        flash('Could not delete zip file on server', 'alert-warning')
        return render_template('misc/genotyping.html', ref_exists=ref_exists, filenames=filenames)

    with zipfile.ZipFile(os.path.join(current_app.config['UPLOAD_FOLDER'], 'genotyping_results.zip'), 'w') as zipf:
        ...
        for file in f:
            ...
            file_wb = load_workbook(os.path.join(current_app.config['UPLOAD_FOLDER'], secure_filename(file.filename)))
            ...
            out_path = os.path.join(current_app.config['UPLOAD_FOLDER'], 'genotyping_results', filename)
        ...
        if not os.listdir(os.path.join(current_app.config['UPLOAD_FOLDER'], 'genotyping_results')):
            delete_files(f)
            flash('The server was not cleaned of generated results files', 'alert-warning')
            return render_template('misc/genotyping.html', ref_exists=ref_exists, filenames=filenames)
        for filename in os.listdir(os.path.join(current_app.config['UPLOAD_FOLDER'], 'genotyping_results')):
            if not filename == '.gitignore':
                os.remove(os.path.join(current_app.config['UPLOAD_FOLDER'], 'genotyping_results', filename))

    # Delete files from server
    delete_files(f)

    try:
        return send_file(os.path.join(current_app.config['UPLOAD_FOLDER'], 'genotyping_results.zip'),
